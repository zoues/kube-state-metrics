...
func RegisterCustomResourceDefinitionCollector(registry prometheus.Registerer, crdclient ClientSet, namespaces []string, opts *options.Options) {
	client := crdclient.CRDClient.RESTClient()

	crdsinfs := NewSharedInformerList(client, "customresourcedefinitions", []string{metav1.NamespaceAll}, &v1beta1.CustomResourceDefinition{})
